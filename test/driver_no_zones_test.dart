...
void main() {
  ...
  test('لا طلب مدفوع بلا سائق: الإسناد يُطلق خادمياً لحظة انقلاب is_paid', () {
    ...
    // والمكنسة الدورية تبقى الضمان الأخير — لا تُحذف، وبإيقاع 5 دقائق.
    expect(fn.contains('sweepUnassignedPaidOrders'), isTrue);
    expect(fn.contains('"every 5 minutes"'), isTrue,
        reason: 'سؤال المالك «لماذا 15 دقيقة» — قُصّرت لخمس، والباقي حدثي');
