...
The Android workflow (`android-release`) mirrors iOS: it declares the same push-to-`main` trigger, builds a signed AAB, and publishes to the Google Play **production** track (`track: production` since 2026-09-21 — owner decision; it was `internal` before, and Codemagic publishes to one track only, so nothing reaches internal testing any more. Revert by setting it back to `internal`). It publishes through the `google_play` variable group in Codemagic (`GCLOUD_SERVICE_ACCOUNT_CREDENTIALS`) — already configured and verified: build #30 (2026-08-31) completed its Publishing step and uploaded to the internal track. Android signing uses `android/key.properties` (gitignored) in local builds and the `android_credentials` group in CI. **The first production release must be promoted by hand once:** the API uploads the bundle fine but `tracks.update` returns `Precondition check failed` until the app has shipped to production at least once — see "فشل خطوة النشر" in `production_deployment_guide.md`, which also records that a `500` from Apple's ContentDelivery during an iOS publish means the upload already succeeded (check TestFlight before re-running, and never blame the build-number logic).
